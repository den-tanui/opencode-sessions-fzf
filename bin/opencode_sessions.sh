...
# ─── Format functions ────────────────────────────────────────────────────────
# Input: id|title|dir|time_updated|name|time_ago (time_ago computed in SQL)
format_session() {
  while IFS='|' read -r id title dir time_updated name time_ago; do
